...
def compose_system_prompt(
    ...
    *,
    ...
    ``base_prompt_override`` is a caller-supplied replacement for the agent
    block only (``AgentInvocationRequest.system_prompt``, §8.79). It is used
    verbatim — it is caller text, not an agent template — and every block
    around it still applies, so no caller can drop the runtime's invariants.
    """

    # A caller-supplied prompt takes the agent block's slot verbatim, so every
    # block around it — platform, tools, per-turn context — still applies.
    agent_block = base_prompt if base_prompt_override is None else base_prompt_override
