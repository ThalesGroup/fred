...
async def _detached_runnable_config() -> AsyncIterator[None]:
    ...
    Full rationale: RUNTIME-EXECUTION-CONTRACT.md §8.82.
    """
...
class LocalRegistryAgentInvoker(AgentInvokerPort):
    ...
    keeps today's behaviour (RUNTIME-EXECUTION-CONTRACT.md §8.75).
    ...
    async def invoke(self, request: AgentInvocationRequest) -> AgentInvocationResult:
        ...
        inherited_turn: dict[str, Any] = (
            ...
            if inherited is not None
            # A cross-agent child inherits nothing from the parent turn, but it
            # still resolves its own capabilities against the pod's registry.
            else {"capability_registry": self._capability_registry}
        )

        final_result: AgentInvocationResult | None = None
        content_parts: list[str] = []
        # `aclosing` is load-bearing: a branch below can still return
        # mid-iteration, and an abandoned generator never runs the child turn's
        # own cleanup. Full rationale: RUNTIME-EXECUTION-CONTRACT.md §8.81.
        async with (
            ...
            async for payload in payloads:
                ...
                if kind == "final":
                    ...
                    # Keep draining: the child turn's own post-`final` events
                    # (its KPI/turn accounting) never run if we return here.
                    continue
...
async def _closing_event_stream(
    ...
    generator skips the `finally` that ends the turn's root span (§8.81).
    """
...
async def _iterate_runtime_event_payloads(
    ...
    *,
    ...
    - all three come from `LocalRegistryAgentInvoker`'s private state, never
      from the request (RUNTIME-EXECUTION-CONTRACT.md §8.75)

    system_prompt_override:
    - the ReAct-family callee's rendered template layer, replaced by the caller
      for this turn (`AgentInvocationRequest.system_prompt`, §8.79). Unlike the
    ...
    try:
        ...
        if isinstance(definition, GraphAgentDefinition):
            ...
            # `finally` ends the turn's root span (see §8.81).
