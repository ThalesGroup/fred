...
@pytest.mark.asyncio
async def test_tool_span_marks_a_cancelled_tool_call_cancelled_not_ok() -> None:
    """`CancelledError` is a BaseException, so it escapes `tool_span`'s
    `except Exception`. Without its own clause the optimistic `status="ok"`
    would survive, and a stopped sub-agent call would read as a success in the
    trace while the audit stream records `outcome="cancelled"`."""
    ...
    assert len(tracer.spans) == 1
    assert tracer.spans[0].attributes.get("status") == "cancelled"
    assert tracer.spans[0].ended is True
