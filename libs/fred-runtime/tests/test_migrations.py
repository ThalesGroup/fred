...
def test_runtime_migration_tree_is_packaged_and_linear() -> None:
    ...
    assert {path.name for path in (RUNTIME_ALEMBIC_DIR / "versions").glob("*.py")} == {
        "__init__.py",
        "a1e2f3c4d5b6_create_session_history.py",
        "b2f3a4e5c6d7_add_exchange_id.py",
        "c3d4b5a6f7e8_add_team_and_instance.py",
    }
    ...
    assert [(revision.revision, revision.down_revision) for revision in revisions] == [
        ("c3d4b5a6f7e8", "b2f3a4e5c6d7"),
        ("b2f3a4e5c6d7", "a1e2f3c4d5b6"),
        ("a1e2f3c4d5b6", None),
    ]
