...
class AgentInvocationRequest(FrozenModel):
    ...
    That block ONLY: the runtime-owned platform blocks, output contract, tool
    descriptions and per-turn context are still composed around it and a caller
    cannot remove them. ReAct-family callees only — a Graph callee composes no
    system prompt, so the invoker refuses rather than ignoring the field.
    """
