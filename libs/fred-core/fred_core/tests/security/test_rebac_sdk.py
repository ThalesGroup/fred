    ...
    *,
    ...
    return SecurityConfiguration(
        ...
        delegation=DelegationConfig(
            enabled=delegation,
            caller_policies=[CallerPolicy(client_id="a-runtime", subject="a-subject")]
            if delegation
            else [],
        ),
        rebac=rebac,
...
async def test_factory_refuses_to_start_under_delegation_until_standing_is_ready(
    ...
    """A receiver refuses to start rather than refusing every person: with the
    gate on and no standing marker, every person decision would be a 403."""
    ...
    with pytest.raises(ValueError, match="Account standing is not ready"):
        await rebac_sdk_factory(_security(delegation=True), kpi_writer=NoOpKPIWriter())
...
async def test_factory_admits_a_ready_store_under_delegation(
    ...
    sdk = await rebac_sdk_factory(
        _security(delegation=True), kpi_writer=NoOpKPIWriter()
    )
