...
def _restore_delegation() -> Iterator[None]:
    installed = delegation.get_delegation_config()
    installed_issuer = delegation._delegation_issuer
    installed_audience = delegation._delegation_audience
    yield
    delegation.initialize_delegation(
        installed, issuer=installed_issuer, audience=installed_audience
    )
...
def _enable(*allowed_callers: str) -> None:
    delegation.initialize_delegation(
        DelegationConfig(
            enabled=True,
            ...
            caller_policies=[
                ...
                for client_id in allowed_callers
            ],
        ),
        issuer=_ISSUER,
        audience=_AUDIENCE,
...
def client() -> TestClient:
    ...
    async def mcp(
        request: Request,
        caller=Depends(oidc.get_authenticated_caller),
    ) -> dict[str, Any]:
        user = await oidc.resolve_request_principal(request, caller, query_only=True)
...
def test_query_grant_from_an_allow_listed_caller_names_the_person(
...
def test_copied_client_claim_or_invalid_token_context_cannot_delegate(
    client: TestClient,
    caller: KeycloakUser,
...
def test_grant_from_a_caller_not_on_the_allow_list_is_ignored_and_audited(
    client: TestClient, audit: _AuditSink
) -> None:
    _enable(_OTHER_CALLER)
    ...
    assert audited["reason"] == "caller_not_allow_listed"
    for canary in (_CALLER, _BEARER, *_GRANT.values()):
        assert canary not in str(audited)


def test_an_allow_listed_caller_without_parameters_acts_for_nobody(
    client: TestClient, audit: _AuditSink
) -> None:
    _enable(_CALLER)
...
def test_a_caller_not_on_the_allow_list_never_has_its_body_read(
    client: TestClient, audit: _AuditSink, body_reads: list[str]
) -> None:
    _enable(_OTHER_CALLER)
...
def test_an_allow_listed_caller_has_its_body_read_for_a_grant(
    client: TestClient, body_reads: list[str]
) -> None:
    _enable(_CALLER)
...
def test_a_partial_query_set_from_a_caller_not_on_the_allow_list_is_not_audited(
    client: TestClient, audit: _AuditSink
) -> None:
    _enable(_OTHER_CALLER)
...
def test_a_partial_body_set_from_an_allow_listed_caller_is_audited(
    client: TestClient, audit: _AuditSink
) -> None:
    _enable(_CALLER)
...
def test_an_unusable_body_from_an_allow_listed_caller_leaves_the_request_alone(
    client: TestClient, audit: _AuditSink
) -> None:
    _enable(_CALLER)
...
# ---------------------------------------------------------------------------
# Flag off, authentication off, and the own-credential guard
# ---------------------------------------------------------------------------


def test_with_the_flag_off_the_dependency_behaves_as_before(
    client: TestClient, audit: _AuditSink
) -> None:
    delegation.initialize_delegation(DelegationConfig())
