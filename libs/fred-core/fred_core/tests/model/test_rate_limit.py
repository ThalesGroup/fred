...
def test_an_unusable_retry_after_is_ignored() -> None:
    for raw in ("garbage", "NaN", "inf", "-inf", "-1"):
        assert is_rate_limit(_WithResponse({"Retry-After": raw}))[1] is None
...
def test_http_date_retry_after_is_supported() -> None:
    ...
    raw = format_datetime(datetime.now(timezone.utc) + timedelta(seconds=30))
    delay = is_rate_limit(_WithResponse({"Retry-After": raw}))[1]
    assert delay is not None and 28 <= delay <= 30
    assert (
        is_rate_limit(_WithResponse({"Retry-After": "Wed, 21 Oct 2015 07:28:00 GMT"}))[
            1
        ]
        == 0
    )
...
def test_explicit_non_429_status_wins_over_message_fallback() -> None:
    exc = _WithResponse()
    exc.response.status_code = 403
    exc.args = ("rate limit 429 denied",)
    assert is_rate_limit(exc) == (False, None)
    assert is_rate_limit(Exception("request 14290 failed")) == (False, None)
