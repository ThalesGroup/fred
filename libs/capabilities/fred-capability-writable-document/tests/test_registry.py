...
fred-runtime is a dev-group (test-only) dependency here, like every sibling
capability package: the boot invariant below is a fred-runtime concern, but
nothing this package ships imports it — the store and the Alembic env read
`storage.postgres` through fred-core.
"""
