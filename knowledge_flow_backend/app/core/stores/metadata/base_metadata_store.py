...
class BaseMetadataStore:
    ...
    def get_all_metadata(self, filters: dict) -> List[DocumentMetadata]:
        ...
        Supports structured filter operations generated by the dynamic filter system.
        Filter fields are named as: field_name for exact match, or field_name__operation for specific operations.

        Available operations are defined by fred_core.store.filters.filter_ops_by_type based on field types:
        - Use generate_filter_model() to see all available operations for your model
        - Common operations: eq (default), lt, lte, gt, gte, in, icontains, contains, overlap

        Examples:
        - {"title": "report"} - exact match  
        - {"title__icontains": "annual"} - case-insensitive substring match
        - {"count__gt": 5} - greater than
        - {"tags__contains": "finance"} - array contains value
        - {"tags__overlap": ["finance", "quarterly"]} - array overlaps with values

        :param filters: dict of structured filter operations
        :return: list of metadata documents matching the filters
        """
        pass
