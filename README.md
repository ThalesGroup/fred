...
| LibreOffice  | Headless doc converter     | [LibreOffice installation instructions](https://www.libreoffice.org/download/download-libreoffice/) | Required for PPTX multimodal ingestion (`pptx -> pdf`) via the `soffice` command                                                                |
...
cd fred
```
> Note: the PPTX multimodal ingestion path in `knowledge-flow-backend` requires LibreOffice to be installed locally and the `soffice` command to be available in `PATH`. On Debian/Ubuntu, this can be installed with `apt install libreoffice`.
