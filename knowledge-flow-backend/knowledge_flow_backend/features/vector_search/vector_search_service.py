...
class VectorSearchService:
    ...
    def _record_search_stats(self, *, base_dims: dict[str, Optional[str]], hits_count: int, top_k: int) -> None:
    ...
    async def _semantic(
        ...
        with self.kpi.timer("rag.search_latency_ms", dims=base_dims, actor=self._kpi_actor()) as kpi_dims:
            try:
        ...
        self._record_search_stats(base_dims=base_dims, hits_count=hits_count, top_k=k)
        if not ann_hits:
            logger.warning(
    ...
    def rerank_documents(self, question: str, documents: List[VectorSearchHit], top_r: int) -> List[VectorSearchHit]:
        ...
        with self.kpi.timer("rag.rerank_latency_ms", dims=base_dims, actor=self._kpi_actor()) as kpi_dims:
