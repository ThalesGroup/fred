...
class FilesystemController:
    ...
    def _register_routes(self, router: APIRouter):
        @router.get("/fs/list", tags=["Filesystem"], summary="List files and directories in the root")
        ...
        @router.get("/fs/stat/{path:path}", tags=["Filesystem"], summary="Get file information")
        async def stat(
            path: str,
            user: KeycloakUser = Depends(get_current_user),
        ):
        ...
        @router.get("/fs/cat/{path:path}", tags=["Filesystem"], summary="Read a file")
        ...
        @router.post("/fs/write/{path:path}", tags=["Filesystem"], summary="Write a file")
        ...
        @router.delete("/fs/delete/{path:path}", tags=["Filesystem"], summary="Delete a file")
        ...
        @router.get("/fs/grep", tags=["Filesystem"], summary="Search files by regex")
        ...
        @router.get("/fs/pwd", tags=["Filesystem"], summary="Get root path of the filesystem")
        async def pwd(user: KeycloakUser = Depends(get_current_user)):
            authorize_or_raise(user, Action.READ, Resource.FILES)
            try:
        ...
        @router.post("/fs/mkdir/{path:path}", tags=["Filesystem"], summary="Create a directory/folder")
