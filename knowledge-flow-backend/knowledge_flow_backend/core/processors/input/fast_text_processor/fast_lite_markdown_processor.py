...
class FastLiteMarkdownProcessor(BaseFastTextProcessor):
    """Fast text processor for Markdown files. Reads the file as-is since it is already plain text."""
    ...
    def extract(self, file_path: Path, options: FastTextOptions | None = None) -> FastTextResult:
        ...
        pages = [FastPageText(page_no=1, text=text, char_count=total_chars)] if opts.return_per_page else []
