...
class LitePdfToMdExtractor(BaseLiteMdProcessor):
    ...
    def _remove_additional_tags(self, text: str, opts: LiteMarkdownOptions) -> str:
        return remove_additional_tags(text) if opts.remove_additional_tags else text
    ...
    def _extract_pymupdf4llm(self, file_path: Path, opts: LiteMarkdownOptions) -> LiteMarkdownResult:
        ...
        for p in pages:
            ...
            md_text = self._normalize_whitespace(text, opts)
            ...
            if opts.max_chars and count_char > opts.max_chars:
                truncated = True
                continue
    ...
    def extract(self, file_path: Path, options: LiteMarkdownOptions | None = None) -> LiteMarkdownResult:
        ...
        try:
            result: LiteMarkdownResult = self._extract_with_markitdown(file_path, opts)
            logger.info(
            ...
            logger.warning(f"Markitdown PDF extraction failed, trying markitdown: {e}")
        
        # Final fallback: return empty result (never return None)
        return LiteMarkdownResult(
            document_name=file_path.name,
            page_count=0,
            total_chars=0,
