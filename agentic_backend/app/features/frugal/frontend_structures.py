...
from app.features.frugal.structure.facts import Fact
from app.features.frugal.structure.workload_scores import WorkloadScores
from app.features.k8.structure import Cluster, WorkloadKind
...
class WorkloadDescription(BaseModel):
    ...
    kind: WorkloadKind = Field(
        ..., description="The kind of workload, either 'Deployment' or 'StatefulSet'"
    )
    facts: List[Fact] = Field(
        default_factory=list, description="The facts about the workload components"
    )
...
class NamespaceDescription(BaseModel):
    ...
    workloads: List[WorkloadDescription] = Field(
        default_factory=list, description="List of resources"
    )
    facts: List[Fact] = Field(
        default_factory=list, description="The facts about the namespace components"
    )
...
class WorkloadScore(BaseModel):
    ...
    scores: Optional[WorkloadScores] = Field(
        default=None, description="The scores of the workload"
    )
...
class ClusterScore(BaseModel):
    ...
    workload_scores: List[WorkloadScore] = Field(
        default_factory=list, description="List of workload scores"
    )
...
class ClusterDescription(BaseModel):
    ...
    cluster: str = Field(default_factory=str, description="The name of the cluster")
    alias: str = Field(default_factory=str, description="The alias of the cluster")
    namespaces: List[NamespaceDescription] = Field(
        default_factory=list, description="List of namespaces"
    )
    facts: List[Fact] = Field(
        default_factory=list, description="The facts about the cluster components"
    )
...
class ClusterFootprint(BaseModel):
    ...
    energy: Observation = Field(
        ..., description="The energy consumption of the cluster."
    )
