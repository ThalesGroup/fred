...
class Cluster(BaseModel):
    ...
    fullname: Optional[str] = Field(
        None, description="The fullname he full name of the observed entity."
    )
    alias: Optional[str] = Field(None, description="The alias of the observed entity.")
    region: Optional[str] = Field(
        None, description="The region of the observed entity."
    )
    provider: Optional[str] = Field(
        None, description="The provider of the observed entity."
    )


class ClusterList(BaseModel):
    clusters_list: List[Cluster] = Field(
...
class NamespacesList(BaseModel):
    cluster: str = Field(
        default_factory=str,
        description="The name of the cluster where the Namespaces reside",
    )
...
class Namespace(BaseModel):
    cluster: str = Field(
        default_factory=str,
        description="The name of the cluster where the Namespace resides",
    )
    ...
    resource_quota: Optional[Dict[str, Dict[str, str]]] = Field(
        default=None,
        description="The resource quota of the namespace associated to their value",
    )
    limitrange_resource: Optional[Dict[str, Dict[str, str]]] = Field(
        default=None,
...
class WorkloadNameList(BaseModel):
    cluster: str = Field(
        default_factory=str,
        description="The name of the cluster where the workloads reside",
    )
    namespace: str = Field(
        default_factory=str, description="The namespace where the workloads are located"
    )
...
class Workload(BaseModel):
    cluster: str = Field(
        default_factory=str,
        description="The name of the cluster where the workload resides",
    )
    namespace: str = Field(
        default_factory=str, description="The namespace where the workload is located"
    )
...
class CustomObjectInfo(BaseModel):
    group: str = Field(
        default_factory=str, description="The group of the custom objects' definition"
    )
    version: str = Field(
        default_factory=str, description="The version of the custom objects' definition"
    )
    plural: str = Field(
        default_factory=str, description="The plural of the custom objects' definition"
    )
    cluster: str = Field(
        default_factory=str,
        description="The name of the cluster where the custom objects reside",
    )
    namespace: str = Field(
        default_factory=str, description="The namespace of the custom objects"
    )
...
class CustomObject(CustomObjectInfo):
    ...
    body: Optional[Dict[str, Any]] = Field(
        default=None, description="The custom object body"
    )
...
class ConfigMapsList(BaseModel):
    cluster: str = Field(
        default_factory=str,
        description="The name of the cluster where the resource resides",
    )
    namespace: str = Field(
        default_factory=str, description="The namespace where the resource is located"
    )
    resource_name: str = Field(
        default_factory=str,
        description="The name of the resource associated with this overview",
    )
    config_maps_list: List[Dict[str, Any]] = Field(
        default_factory=str,
...
class ServicesList(BaseModel):
    ...
    services_list: List[Dict[str, Any]] = Field(
        default_factory=str,
...
class IngressesList(BaseModel):
    ...
    ingresses_list: List[Dict[str, Any]] = Field(
        default_factory=str,
...
class Rule(BaseModel):
    ...
    settings: Settings = Field(
        description="The rule's settings. Must include the key 'value'."
    )
    ...
    def check_value_based_on_rule_type(self):
        ...
        if self.type == RuleType.DATE:
            try:
                ...
                raise ValueError(
                    f"For type 'date', 'value' must be a valid ISO date. Received: {value}"
                )
            ...
            try:
                ...
                raise ValueError(
                    f"For type 'cron', 'value' must be a valid cron expression. Received: {value}"
                )
