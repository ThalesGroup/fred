...
from app.common.structures import DAOTypeEnum
...
class KubeService:
    ...
    def get_namespaces_list(self, cluster: str) -> NamespacesList:
        ...
        """
        try:
            logger.debug("Reading file for the list of Namespaces, cluster=%s", cluster)
            return self.dao.loadCacheItem(NamespacesList, cluster)
        except FileNotFoundError:
            namespaces = self.connected_client.get_namespaces_list(cluster)

            logger.debug(
                "Updating file for the list of Namespaces, cluster=%s",
                namespaces.cluster,
            )
    ...
    def get_namespace_description(self, cluster: str, namespace: str) -> Namespace:
        ...
        try:
            ...
            description = self.connected_client.get_namespace_description(
                cluster, namespace
            )
    ...
    def get_workload_names_list(
        self, cluster: str, namespace: str, kind: WorkloadKind
    ) -> WorkloadNameList:
        """
        ...
        try:
            logger.debug(
                "Reading file for the list of the workloads, cluster=%s, namespace=%s",
                cluster,
                namespace,
            )
            return self.dao.loadCacheItem(WorkloadNameList, cluster, namespace, kind)
        except FileNotFoundError:
            workload_names_list = self.connected_client.get_workload_names_list(
    ...
    def get_workload_description(
        self, cluster: str, namespace: str, workload_name: str, kind: WorkloadKind
    ) -> Workload:
        """
        ...
        try:
            ...
            return self.dao.loadCacheItem(
                Workload, cluster, namespace, kind, workload_name
            )
        except FileNotFoundError:
            description = self.connected_client.get_workload_description(
                cluster, namespace, workload_name, kind
            )
            ...
            self.dao.saveCache(description, cluster, namespace, kind, workload_name)

            return description

    def get_custom_object_description(
        ...
        try:
            logger.debug(
                f"Reading file for the description of the custom object, "
            ...
            custom_object_with_body = (
                self.connected_client.get_custom_object_description(custom_object)
            )
            self.__update_custom_object_description(custom_object_with_body)
            return custom_object_with_body

    def get_custom_object_description_list(
        self, custom_object_info: CustomObjectInfo
    ) -> List[CustomObject]:
        """
        ...
        try:
            logger.debug(
                f"Reading file for the description of the custom object list, "
            ...
            return self.connected_client.get_custom_object_description_list(
                custom_object_info
            )

    def get_workload_configmaps(
        self, cluster: str, namespace: str, workload_name: str, kind: WorkloadKind
        ...
        try:
            ...
            return self.dao.loadCacheItem(
                ConfigMapsList, cluster, namespace, kind, workload_name
            )
        except FileNotFoundError:
            config_maps = self.connected_client.get_workload_configmaps(
                cluster, namespace, workload_name, kind
            )
            logger.debug(
            ...
            self.dao.saveCache(config_maps, cluster, namespace, kind, workload_name)

            return config_maps

    def get_workload_services(
        self, cluster: str, namespace: str, workload_name: str, kind: WorkloadKind
    ) -> ServicesList:  # pylint: disable=R0913, R0917
        """
        ...
        try:
            ...
            return self.dao.loadCacheItem(
                ServicesList, cluster, namespace, kind, workload_name
            )
        except FileNotFoundError:
            services_list = self.connected_client.get_workload_services(
                cluster, namespace, workload_name, kind
            )
            logger.debug(
    ...
    def get_workload_ingresses(
        ...
        try:
            ...
            return self.dao.loadCacheItem(
                IngressesList, cluster, namespace, kind, workload_name
            )
        except FileNotFoundError:
            ingresses_list = self.connected_client.get_workload_ingresses(
                cluster, namespace, workload_name, kind
            )
            logger.debug(
    ...
    def __update_custom_object_description(self, custom_object: CustomObject):
        ...
        """
        logger.debug(
            f"Updating file for the description of the custom object, "
            f"cluster={custom_object.cluster}, namespace={custom_object.namespace}, "
            f"custom_object_name={custom_object.name}, group={custom_object.group}, "
            f"version={custom_object.version}, plural={custom_object.plural}"
        )
...
class ConnectedKubeService:
    ...
    def _setup_service(self):
        ...
        )  # Global configuration loading for kube clients
    ...
    def _get_kube_client(self, cluster: str, api_cls):
        ...
        if cluster not in self.api_clients:
            logger.debug(f"Client does not exist, creating one for Cluster '{cluster}'")
            api_client = self._load_kube_config()
            self.api_clients[cluster] = api_client
        else:
            logger.debug(f"Client for Cluster '{cluster}' already exists, using it")
        ...
        try:
            ...
            for kube_config_context in list_kube_config_contexts:
                ...
                try:
                    ...
                    logger.error(f"Error loading Cluster '{cluster}': {cluster_error}")
                    continue
        except Exception as e:
            traceback.print_exc()
            raise e
    ...
    def parse_cluster(self, cluster: str) -> (str, str, str):
        ...
        try:
            ...
            # providers/Microsoft.ContainerService/managedClusters/<cluster-name>
                ...
                "/subscriptions/" in cluster and "Microsoft.ContainerService" in cluster
            ):
                match = re.search(r"managedClusters/([a-zA-Z0-9\-]+)", cluster)
                if match:
    ...
    def load_cluster_context(self, cluster: str):
        ...
        if cluster not in {c.fullname for c in self.get_clusters_list().clusters_list}:
            raise ValueError(
    ...
    def _get_workload(
        ...
        match kind:
            case WorkloadKind.DEPLOYMENT:
                v1 = self._get_kube_client(cluster, client.AppsV1Api)
                return v1.read_namespaced_deployment(
                    name=workload_name, namespace=namespace
                )
            case WorkloadKind.STATEFUL_SET:
                v1 = self._get_kube_client(cluster, client.AppsV1Api)
                return v1.read_namespaced_stateful_set(
                    name=workload_name, namespace=namespace
                )
            case WorkloadKind.DAEMON_SET:
                v1 = self._get_kube_client(cluster, client.AppsV1Api)
                return v1.read_namespaced_daemon_set(
                    name=workload_name, namespace=namespace
                )
            case WorkloadKind.JOB:
                v1 = self._get_kube_client(cluster, client.BatchV1Api)
    ...
    def get_workload_description(
        ...
        return Workload(
            cluster=cluster, namespace=namespace, object=workload.to_dict(), kind=kind
        )
    ...
    def get_custom_object_description(
        ...
        custom_object_client = self._get_kube_client(
            custom_object.cluster, client.CustomObjectsApi
        )
    ...
    def get_custom_object_description_list(
        ...
        custom_object_client = self._get_kube_client(
            custom_object_info.cluster, client.CustomObjectsApi
        )
        ...
        return [
            CustomObject(**d, name=body["metadata"]["name"], body=body)
            for body in custom_object_bodies_list
        ]
    ...
    def get_workload_configmaps(
        ...
        configmap_objects_list: List[Dict[str, Any]] = (
            self._get_configmaps_from_volumes_spec(cluster, namespace, volumes)
        )
        return ConfigMapsList(
            cluster=cluster,
            namespace=namespace,
            resource_name=workload_name,
            config_maps_list=configmap_objects_list,
            kind=kind,
        )
    ...
    def get_workload_services(
        ...
        service_objects_list: List[Dict[str, Any]] = (
            self.__get_services_from_template_spec(cluster, namespace, metadata_labels)
        )
        return ServicesList(
            cluster=cluster,
            namespace=namespace,
            resource_name=workload_name,
            services_list=service_objects_list,
            kind=kind,
        )
    ...
    def get_workload_ingresses(
        ...
        services_list = self.get_workload_services(
            cluster, namespace, workload_name, kind
        ).services_list
        ...
        if len(services_list) == 0:
            ...
            ingresses_list = [
                ...
                if has_matching_service(ingress)
            ]
    ...
    @requires_online
    def _get_configmaps_from_volumes_spec(
        self, cluster: str, namespace: str, volumes
    ) -> List[Dict[str, Any]]:
        """
    ...
    def __get_services_from_template_spec(
        self, cluster: str, namespace: str, metadata_labels: Dict[str, str] | None
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for service in list_service_name.items:
                ...
                if service_selector is not None and set(
                    service_selector.items()
                ).issubset(set_metadata_labels):
                    service_name = service.metadata.name

                    service_object = core_v1.read_namespaced_service(
...
def mask_sensitive_data(configmap: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(data, dict):
        ...
        logger.debug(
            "Skipping configmap with no or invalid 'data': %s",
