...
def upgrade() -> None:
    ...
    op.execute("UPDATE session SET updated_at = CURRENT_TIMESTAMP WHERE updated_at IS NULL")
    ...
    with op.batch_alter_table('session', schema=None) as batch_op:
        ...
        batch_op.alter_column('session_data', existing_type=sa.JSON(), type_=jsonb_type, nullable=False)
...
def downgrade() -> None:
    ...
    with op.batch_alter_table('session', schema=None) as batch_op:
        ...
        batch_op.alter_column('session_data', existing_type=jsonb_type, type_=sa.JSON().with_variant(sa.JSON(), 'sqlite'), nullable=True)
