...
from agentic_backend.agents.rags.graph_rag_expert import Richard
...
class RAGEvaluator(BaseEvaluator):
    async def run_evaluation(
        ...
        agent = await self.setup_agent(
            agent_type=Richard, agent_name=agent_name, doc_lib_ids=doc_lib_ids
        )
...
def main():
    ...
    exit_code = asyncio.run(evaluator.main(agent_name="Richard"))
    sys.exit(exit_code)
