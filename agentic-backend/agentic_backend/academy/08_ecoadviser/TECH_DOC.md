...
Utilisateur ↔ LLM GPT-4o (raisonneur principal)
                   ...
                   └─ MCP Geo Distance → Nominatim + OSRM (géocodage / distances)
```
...
| MCP Geo Distance (`mcp-geo-service`, `http://127.0.0.1:9801/mcp`) | FastAPI locale combinant Nominatim (géocodage) et OSRM (routing). | `geocode_location`, `compute_trip_distance`, `estimate_trip_between_addresses`. | Distances/temps réalistes entre deux adresses, avec fallback haversine + transparence sur la source. |
...
5. **Calculer la distance réelle** : si l’utilisateur fournit uniquement des adresses, `estimate_trip_between_addresses` renvoie lat/lon, distance routière OSRM et fallback géodésique. Le résultat nourrit ensuite le calcul CO₂.
...
- **Service Geo/OSRM indisponible** : fallback immédiat sur la distance géodésique (haversine) et mention explicite dans la réponse.
...
EcoAdvisor associe donc un LLM GPT-4o, les données locales DuckDB et quatre APIs publiques (ADEME, trafic Grand Lyon, RDATA TCL, Nominatim/OSRM) afin de produire un bilan CO₂ contextualisé, sourcé et directement exploitable par les équipes métier.
