...
from agentic_backend.core.chatbot.chat_schema import TextPart
...
class Travel(AgentFlow):
    ...
    async def parse_city_and_category_node(
        ...
        return {**state, "city": city, "category_tag": category_tag}
    ...
    async def osm_search_node(self, state: TravelAgentState) -> TravelAgentState:
        ...
        if not city:
            ...
            return {**state, "osm_results": [], "lat": None, "lon": None}
        ...
        return {**state, "osm_results": results, "lat": lat, "lon": lon}
    ...
    async def format_pois_node(self, state: TravelAgentState) -> TravelAgentState:
        ...
        if not lat or not lon:
            ...
            final_text = "*Aucun point d'intérêt trouvé.*"
        else:
