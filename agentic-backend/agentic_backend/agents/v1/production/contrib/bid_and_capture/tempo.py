...
RAG_TUNING = AgentTuning(
    role="Analyste fichiers joints",
    description=(
        "Agent RAG expert pour les réponses aux appels d'offres Thales. Tempo analyse, sélectionne et synthétise "
        ...
        "Tempo s'appuie prioritairement sur les extraits de documents fournis.  \n"
    ...
    fields=[
        FieldSpec(
            ...
            required=True,
            default=(
                "Tu es Tempo, un expert Thales spécialisé dans l'analyse documentaire et la rédaction de réponses aux appels d'offres (RAO).\n"
                "\n"
        ...
        FieldSpec(
            ...
            type="boolean",
            title="Document libraries picker (disabled)",
            description="Tempo ne recherche pas dans les bibliothèques documentaires (corpus).",
            required=False,
            default=False,
            ui=UIHints(group="Chat options"),
        ),
        ...
        FieldSpec(
            ...
            type="boolean",
            title="RAG scope selector (attachments only)",
            description="Tempo est limité aux pièces jointes de session.",
            required=False,
            default=False,
            ui=UIHints(group="Chat options"),
        ),
        ...
        FieldSpec(
            ...
            description="Doit rester désactivé: Tempo n'interroge jamais le corpus.",
            required=False,
            default=False,
            ui=UIHints(group="Chat options"),
        ),
...
@expose_runtime_source("agent.Tempo")
class Tempo(AgentFlow):
    """
    ...
    def _render_tuned_prompt(self, key: str, **tokens) -> str:
        ...
        if not prompt:
            logger.warning("Tempo: no tuned prompt found for %s", key)
            raise RuntimeError(f"Tempo: no tuned prompt found for '{key}'.")
    ...
    def _system_prompt(self) -> str:
        ...
        logger.debug(
            "Tempo: resolved system prompt (len=%d, language=%s)",
            len(sys_text),
    ...
    async def _expand_with_keywords(self, question: str) -> Tuple[str, List[str]]:
        ...
        try:
            ...
            logger.warning(
                "Tempo: keyword expansion failed, using raw question. err=%s", e
            )
            return question, []
    ...
    async def _run_reasoning_step(self, state: MessagesState):
        ...
        try:
            ...
            logger.debug(
                "[AGENT] Tempo prompt check: response_language=%s include_chat_context=%s system_prompt=%r chat_context=%r",
                response_language,
            ...
            if skip_rag:
                logger.debug("Tempo: general-only mode; bypassing retrieval.")
            ...
            if (
                ...
                logger.warning(
                    "Tempo: runtime_context.session_id is missing; attached-file retrieval will be skipped."
                )
