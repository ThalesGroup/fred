...
class InMemorySchedulerEventBus(SchedulerEventBus):
    ...
    def _register(self, task_id: str) -> asyncio.Queue[SchedulerTaskEvent]:
        queue: asyncio.Queue[SchedulerTaskEvent] = asyncio.Queue(maxsize=self._max_queue_size)
        self._subscribers.setdefault(task_id, []).append(queue)
        return queue

    def _unregister(self, task_id: str, queue: asyncio.Queue[SchedulerTaskEvent]) -> None:
