...
from ..user_models import UserRow, GcuVersionsType
...
class PostgresUserStore(BaseUserStore):
  ...
  async def find_user_by_id(self, user_id: UUID, session: AsyncSession | None = None) -> Optional[UserRow]:
  ...
  async def update_gcu_version(self, user_id: UUID, gcu_version: GcuVersionsType,
                               session: AsyncSession | None = None) -> None:
    async with use_session(self._sessions, session) as s:
      user = await s.get(UserRow, user_id)

      if user is None:
        user = UserRow(id=user_id, gcuVersionAccepted=gcu_version, gcuAcceptedAt=datetime.now(timezone.utc))
        s.add(user)
      else:
