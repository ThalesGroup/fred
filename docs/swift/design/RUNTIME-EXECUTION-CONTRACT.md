...
the 429 detector itself moved to `fred_core` in §8.77 and is tested there),
...
`system_prompt` replaces the callee's rendered agent template — that block
only. The platform blocks, output contract, tool descriptions and per-turn
context around it are still composed and no caller can drop them. This is a
deliberate, public widening: any Graph node may run any agent registered in the
pod with its authored template replaced. ReAct-family callees only — a
Graph callee composes no system prompt and is refused. Full entry: §8.79.

`AgentInvocationResult.token_usage` (`dict[str, int] | None`, §8.80) carries the
...
- **The result carries what the callee cited and produced, not only its text.**
  ...
  `final` event (§8.78) — a callee is a full agent, so a caller that renders
...
- **Composition depth / cycle limit.** Narrowed, not closed (§8.75): the
...
- **Agent-scoped checkpoint isolation.** Checkpoint state is keyed by
  ...
  same-agent children, which run checkpointer-free (§8.75); still open for
...
- **A child can never pause for a human.** No longer a hang risk — at
  ...
  interrupted (§8.76) — but still a capability limit: work that genuinely
...
### 8.75 ✅ Sub-agent invocation: same-agent children, bounded depth, `execution_error` mapped — issue #2525 (2026-09-03)
...
> §8.76 (a child can never hang on human approval), §8.78 (the callee's
> `sources`/`ui_parts`), §8.79 (`system_prompt`), §8.80 (per-child token
> spend). §8.81 then fixed what this slice left detached rather than
...
> built; read §14 plus §8.76–§8.81 for what it does today.
...
**Superseded by §8.76**: such tools are now hidden from a child's model, or
...
unchanged *here*. **All four shipped within the day**, in §8.76, §8.80, §8.78
and §8.79 respectively; §14 now carries the widened request and result shapes
...
### 8.76 ✅ A sub-agent can never hang on human approval — issue #2526 (2026-09-03)

Closes the follow-up §8.75 left open ("a child cannot be resumed, and
...
checkpointer-free (§8.75). An `interrupt()` there has nowhere to persist and no
...
`FredHitlMiddleware`. It is never read from a request, exactly like §8.75's
...
### 8.77 ✅ Provider rate limits (429) are retried, not fatal — issue #2535 (2026-09-04)
...
That remains issue #2531 (see §8.75's closing note); this incident is a data
point for it.

### 8.78 ✅ A callee's `sources` and `ui_parts` reach its caller — issue #2529 (2026-09-03)
...
(§8.75) and never streams or persists a turn of its own — its events are
...
### 8.79 ✅ `AgentInvocationRequest.system_prompt` — a caller replaces the callee's template layer — issue #2527 (2026-09-03)

`AgentInvocationRequest` gains `system_prompt: str | None = None` (additive —
the request-side half of §14's widening; §8.80 adds the result-side half).
When set, it replaces **the agent block** of the callee's composed system
prompt — the rendered agent template, the last static block since §8.67 — and
nothing else.

**Every block around it stays runtime-owned.** `compose_system_prompt`
(`react_prompting.py`) takes the override as `base_prompt_override` and still
composes the two platform-wide blocks (§8.70), the global base output
contract, the runtime tool descriptions, any runtime-specific suffixes, and
the per-turn user context below it. A caller can change what the callee *is
asked to do*; it cannot remove what the platform requires of it. The override
is used verbatim — it is caller text, not an agent template, so it goes
through no token substitution.
...
with its authored template replaced. That was accepted rather
...
`PortableContext`: it is caller-supplied per call, and the blocks it cannot
...
### 8.80 ✅ Sub-agent token spend is measurable per child — issue #2528 (2026-09-03)
...
### 8.81 ✅ A sub-agent turn is a nested, spanned subtree in the trace — issue #2525 follow-up (2026-09-04)
...
and `agent_instance_id` are identical by design for a same-agent child (§8.75),
...
### 8.82 ✅ A child turn runs detached from its caller's runnable config — issue #2525 follow-up (2026-09-07)
...
value it needs is passed explicitly (§8.75), so nothing else is lost by
...
separate contextvar, so the nesting §8.81 established still holds.
