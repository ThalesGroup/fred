...
and the monitoring/`tools` pages (`Kpis`, `Runtime`, `McpHub`) — real pages
with their own legacy UI kit, not pure relocations.
...
#2296; `RebacBackfill`'s and `DataHub`'s pages went with them and their
backend endpoints — `POST /tags/rebac/backfill` and
