...
## §RUNTIME-10 — Deep agent dispatch made structural

**ID:** RUNTIME-10  
**RFC:** `docs/swift/rfc/DEEP-AGENT-EXECUTION-RFC.md`  
**Execution:** TBD  
**Status:** `[ ]` Proposed — awaiting developer confirmation

### Goal

`ExecutionCategory.DEEP` / `DeepAgentDefinition` / `DeepAgentRuntime` exist but are unreachable:
`_iterate_runtime_event_payloads` never checks for `DeepAgentDefinition`, so the first agent that
uses it would silently fall through to `ReActRuntime`. Fix dispatch at the root — off
`execution_category`, not another `isinstance` branch bolted before the ReAct fallthrough — and
close the test gap that let this ship unnoticed.

### Deliverables

- [ ] **Category-driven dispatch**
      Replace the `isinstance` fallthrough in `_iterate_runtime_event_payloads` with a dispatch
      keyed on `definition.execution_category`, so subtype-before-supertype correctness is
      structural rather than comment-enforced branch order.
      **Files:** `libs/fred-runtime/fred_runtime/app/agent_app.py`

- [ ] **Shared streaming helper**
      Extract the ReAct-style streaming/validation block (`ReActInput` construction, `executor.stream`
      loop, dict-payload validation) into one helper used by both the ReAct and Deep branches, since
      `DeepAgentRuntime` only overrides `build_executor`.
      **Files:** `libs/fred-runtime/fred_runtime/app/agent_app.py`

- [ ] **Contract doc entry**
      Add a dated §8.13 entry to `RUNTIME-EXECUTION-CONTRACT.md` documenting the dispatch contract
      and the shared ReAct/Deep transport layer.
      **Files:** `docs/swift/design/RUNTIME-EXECUTION-CONTRACT.md`

- [ ] **Offline test**
      Construct a real `DeepAgentDefinition` and assert it dispatches to `DeepAgentRuntime` through
      the actual `_iterate_runtime_event_payloads` path (existing tests monkeypatch this function
      wholesale and never exercise the real branch).
      **Files:** `libs/fred-runtime/tests/test_agent_app.py`

### Non-goals

- No new `ExecutionCategory` values and no change to `DeepAgentDefinition`'s public shape.
- Does not touch `QUALITY-01`/R1b's `agent_app.py` router-extraction refactor directly — sequence
  with it, do not duplicate it.

---

## §RUNTIME-11 — Web search tool + Report Generator agent

**ID:** RUNTIME-11  
**RFC:** `docs/swift/rfc/DEEP-AGENT-EXECUTION-RFC.md`  
**Execution:** TBD  
**Status:** `[ ]` Proposed — awaiting developer confirmation, depends on RUNTIME-10

### Goal

Ship the first concrete deep agent: a DuckDuckGo-backed web search tool and a Report Generator
agent, built on the existing in-process MCP toolkit registry instead of a competing mechanism.

### Deliverables

- [ ] **`web_search` provider in the existing registry**
      Register a new in-process MCP toolkit provider (`ddgs`-backed, no API key) into
      `inprocess_toolkit_registry.build_inprocess_toolkit(provider, agent)`, alongside
      `kf_vector_search` — not a second `inprocess_toolkit_factory` parameter on
      `create_agent_app`.
      **Files:** `libs/fred-runtime/fred_runtime/integrations/inprocess_toolkit_registry.py`

- [ ] **MCP catalog entry**
      Add `mcp-web-search` (`transport: inprocess`, `provider: web_search`, `auth_mode: no_token`).
      **Files:** `apps/fred-agents/config/mcp_catalog.yaml`

- [ ] **Report Generator agent**
      Add `ReportGeneratorDefinition` (first concrete `DeepAgentDefinition` subclass), registered
      in the pod registry, with `default_mcp_servers` referencing `mcp-web-search` plus the
      existing knowledge-flow text/corpus servers.
      **Files:** `apps/fred-agents/fred_agents/report_generator.py`, `apps/fred-agents/fred_agents/registry.py`

- [ ] **Pooled search client**
      Reuse a single `DDGS` client instead of constructing one per tool call.
      **Files:** `apps/fred-agents/fred_agents/web_search.py`

- [ ] **Generated frontend client**
      Regenerate `agenticOpenApi.ts` (do not hand-edit) so `ExecutionCategory` includes `"deep"`.
      Blocked on fixing the agentic-backend OpenAPI codegen path (`agenticOpenApiConfig.json`
      currently points at a nonexistent `agentic-backend/openapi.json` with no Makefile target).
      **Files:** `apps/frontend/src/slices/agentic/agenticOpenApi.ts`, `apps/frontend/src/slices/agentic/agenticOpenApiConfig.json`

- [ ] **Offline tests**
      Cover `web_search` provider resolution, `ReportGeneratorDefinition` registration, and that
      `kf_vector_search` still resolves correctly for the same pod (regression guard for the
      registry-conflict bug this replaces).

### Non-goals

- Do not build a pluggable search-backend abstraction now — `ddgs` only, revisit if needed.
- Do not widen ReBAC/session scoping — web search results carry no conversation/document
