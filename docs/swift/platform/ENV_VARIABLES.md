...
| `FRONTEND_THEME_URL`             | frontend server runtime config        | Zip archive of branding assets (`images/`, `contrib/<brand>/`, root `*.md`) fetched by the container at startup and served over the baked files. `http(s)://` or `file://`; unset disables the overlay. See `apps/frontend/README.md`, "Theme overlay". |
| `FRONTEND_THEME_S3_ACCESS_KEY`   | frontend server runtime config        | With `FRONTEND_THEME_S3_SECRET_KEY`, SigV4-signs the theme request so a private S3-compatible bucket works with a plain object URL. Unset by default; dropped from the nginx environment once the theme is installed. |
| `FRONTEND_THEME_S3_SECRET_KEY`   | frontend server runtime config        | Secret half of the theme S3 key, see above. Inject from a Kubernetes Secret. |
| `FRONTEND_THEME_S3_REGION`       | frontend server runtime config        | Region used in the SigV4 signature; defaults to `us-east-1`, ignored by S3-compatible stores. |
| `FRONTEND_THEME_REQUIRED`        | frontend server runtime config        | Strict `true`/`false`; defaults to `false` (a theme fetch or unpack failure logs one warning and the stock assets are served). `true` makes the container exit instead. |
