...
is rejected. With `security.delegation.accept_delegated_calls`, a workload token
addressed to `security.delegation.audience` is also accepted when it carries the
delegation caller role. `security.m2m` is this backend's Keycloak identity for outbound
