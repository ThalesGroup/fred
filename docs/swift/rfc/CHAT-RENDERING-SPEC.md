...
- les fragments de prompt partagés entre plusieurs pods agents vivent dans une
  lib sous `libs/`, côté `fred-sdk` : c'est la **source unique** du contenu
  (`GLOBAL_BASE_PROMPT_RESOURCES` / `GLOBAL_BASE_PROMPT_MARKDOWN`)
- depuis **RUNTIME-09 (2026-06-29)**, ces fragments ne sont plus composés dans
  les prompts par défaut à l'authoring : ils sont **injectés au runtime** comme
  suffixe non éditable (`build_global_base_prompt_suffix`) dans `ReActRuntime` et
  `DeepAgentRuntime`, à côté de `agent_instructions` MCP et des guardrails
- ils n'apparaissent donc plus dans le `system_prompt_template` éditable (ni dans
  l'éditeur d'agent) et s'appliquent même quand l'opérateur remplace le prompt
- les agents Graph (mindmap, `GraphRuntime`) ne passent pas par ce chemin de
  suffixe et ne reçoivent pas le contrat

Important : le contrat de sortie Mermaid parse-safe est désormais traité comme
une **injection runtime obligatoire et non overrideable**, au même titre que les
contraintes liées à un outil (`agent_instructions`) — voir
`RUNTIME-EXECUTION-CONTRACT.md §8.12` et `SDK-V2-RFC.md §18.4` (révisé).
...
| `libs/fred-sdk/fred_sdk/resources/prompts.py`                                                  | Loader Markdown + source unique du global base prompt (`GLOBAL_BASE_PROMPT_MARKDOWN`) |
| `libs/fred-sdk/fred_sdk/resources/prompts/mermaid_output_contract.md`                          | Contrat de sortie Mermaid parse-safe — injecté au runtime (RUNTIME-09)                |
| `libs/fred-runtime/fred_runtime/react/react_prompting.py`                                      | `build_global_base_prompt_suffix()` — injection runtime du contrat Mermaid (RUNTIME-09) |
