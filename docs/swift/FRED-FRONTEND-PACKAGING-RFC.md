...
The `@fred-oss` scope and three `0.1.0-alpha.1` coordinates below were selected by maintainers and are published on npm. The first release and read-only registry verification are recorded in [`libs/frontend/RELEASE.md`](../../libs/frontend/RELEASE.md). Package API, SDK protocol, release, and enduring publishing owners are named in the release contract; organization ownership alone did not establish those roles.
...
| 5. Prepare and publish prereleases       | Record the selected `@fred-oss` coordinates and bootstrap authority; publish exact validated bytes and verify registry integrity/provenance | The three first prereleases and genuine registry-installed verification are complete; subsequent publication automation and trust/revocation evidence remain separate work |
