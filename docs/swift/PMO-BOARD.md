...
| `FILES-04` | `UNIFIED-FILESYSTEM` | Dimitri | En cours — **product-complete (G1–G5) le 2026-06-24**: routage agents + isolation runtime (G1), provenance dérivée du chemin + badges (G4), UI 4 racines + racine Agents, partage humain par copie vers Espace d'équipe (G5). Reste: helpers SDK (G7), dédup libellés agents identiques (G6), durcissement KF côté principal (G1b, différé agents de confiance) | [CHAT-UI-BACKLOG §4.6](backlog/CHAT-UI-BACKLOG.md) | [AGENT-FILESYSTEM-RFC](rfc/AGENT-FILESYSTEM-RFC.md) | branche `…-final` |
