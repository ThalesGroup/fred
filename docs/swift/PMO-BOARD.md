...
| `FILES-06` | `GCS-STORAGE-BACKEND` | Dimitri | En cours — backend GCS natif (ADC/Workload Identity) pour content store + VFS livré (code + tests + config + Helm + guide GKE) ; reste validation live-bucket sur GKE | [BACKLOG §Phase STORAGE](backlog/BACKLOG.md) | [DEPLOYMENT_GUIDE_GKE](platform/DEPLOYMENT_GUIDE_GKE.md) | branch `1795-…-native-google-cloud-storage-backend` ; GitHub issue #1795 |
