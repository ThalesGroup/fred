...
| `FILES-01` | `AGENT-FILESYSTEM` | Dimitri | En cours — RFC consolide 2026-06-24: UX 4 racines, SDK simple, sorties agents dans Agents | [CHAT-UI-BACKLOG §4.5](backlog/CHAT-UI-BACKLOG.md) | [AGENT-FILESYSTEM-RFC](rfc/AGENT-FILESYSTEM-RFC.md) | `TBD` |
...
| `FILES-04` | `UNIFIED-FILESYSTEM` | Dimitri | En cours — **v1 feature-complete (G1–G7) le 2026-06-24**, prêt pour test bout-en-bout: routage agents + isolation runtime (G1), provenance + badges (G4), UI 4 racines + racine Agents, partage humain par copie (G5), dédup libellés (G6), helpers SDK read_user/read_team + correctif resolve_template (G7). Reste uniquement des reports volontaires: G1b (durcissement KF côté principal, agents de confiance) et raffinements G5/read_user | [CHAT-UI-BACKLOG §4.6](backlog/CHAT-UI-BACKLOG.md) | [AGENT-FILESYSTEM-RFC](rfc/AGENT-FILESYSTEM-RFC.md) | branche `…-final` |
| `FILES-05` | `BIG-FILE-TRANSFER` | Dimitri | Backlog — `/fs` bufferise le fichier entier en mémoire (pas de streaming), OOM-prone sur gros fichiers. Re-ouvre le choix streaming proxy vs presigned MinIO pour les gros fichiers uniquement ; nécessite un court RFC | [CHAT-UI-BACKLOG §4.7](backlog/CHAT-UI-BACKLOG.md) | [AGENT-FILESYSTEM-RFC §9](rfc/AGENT-FILESYSTEM-RFC.md) | `TBD` |
| `FILES-06` | `GCS-STORAGE-BACKEND` | Dimitri | En cours — backend GCS natif livré ; URLs signées V4 internes (IAM signBlob, sans clé) implémentées pour les lectures tabulaires Parquet, avec validation fail-fast au démarrage et masquage des URLs dans les erreurs ; reste la validation live GKE (gate §1bis du RFC) | [BACKLOG §Phase STORAGE](backlog/BACKLOG.md) | [GCS-TABULAR-SIGNED-URL-RFC](rfc/GCS-TABULAR-SIGNED-URL-RFC.md) | branch `1795-…-native-google-cloud-storage-backend-signed` ; GitHub issue #1795 |
...
| `VALID-02` | `ADMIN-SELF-TEST` | Dimitri | En cours — prototype livré (backend + page admin `/admin/self-test`), **run docker-compose vert de bout en bout** ; a détecté + corrigé un bug RAG global (API KF cherchait l'index `embeddinggemma` alors que le worker écrit `vector-index-mistral`). Reste CronJob GKE ~2h + pilotage des vrais composants (RFC §3.2) | [BACKLOG §3b.7](backlog/BACKLOG.md) | [ADMIN-SELF-TEST-HARNESS-RFC](rfc/ADMIN-SELF-TEST-HARNESS-RFC.md) | GitHub issue #1828 |
| `VALID-03` | `SELF-TEST-CORPUS` | Dimitri | En cours — corpus doré (deux bibliothèques, fait-marqueur unique, assertions sur le doc cité) semé dans l'équipe synthétique `fred-selftest` | [BACKLOG §3b.7](backlog/BACKLOG.md) | [ADMIN-SELF-TEST-HARNESS-RFC §4](rfc/ADMIN-SELF-TEST-HARNESS-RFC.md) | branch `swift` |
...
| `EVAL-02` | `EVAL-TASK-EVENTS` | Dimitri | Proposé — RFC à confirmer ; UI `/rework` livrée, reste cutover task-event (fred-core kind `evaluation` + tray multi-source) | [AGENT-EVALUATION-BACKLOG §EVAL-02](backlog/AGENT-EVALUATION-BACKLOG.md) | [AGENT-EVALUATION-TASK-EVENT-AMENDMENT-RFC](rfc/AGENT-EVALUATION-TASK-EVENT-AMENDMENT-RFC.md) | branch `odelia` |
...
| `CHAT-14`  | Dimitri     | Clos   | [CHAT-UI-BACKLOG §15](backlog/CHAT-UI-BACKLOG.md)   | GitHub PR `#1816` (merged) — contribution externe @pdubey28-sketch                                          | Labels d'appels d'outils lisibles dans la trace + durcissement confidentialité: `humanizeToolName` transforme les identifiants MCP bruts en libellés d'action; la trace n'expose plus les noms d'outils, arguments ni payloads de résultat (action + statut + latence uniquement); dédoublonnage des `tool_call` par `call_id`. Tests `traceUtils.test.ts`. (PR auto-étiquetée CHAT-12 par erreur; enregistrée en CHAT-14, CHAT-13 réservé à #1819.) Suivi: retirer le code mort `summarizeToolResultCompact`/`toolResultContent`. |
