...
- [ ] 1.2 Derive a document's identity deterministically from its library and source key, so a second write to the same key updates one document rather than creating another. This derivation is used by the new surface only — the existing ingestion path keeps minting a fresh identity per upload, which is its documented behaviour and is not in scope. Verify with tests that writing the same key twice leaves exactly one document carrying the later content, that the two surfaces coexist on the same library without either disturbing the other, and that a document written by the existing surface is never adopted by a source key
