...
- Frontend: the charter page replaces the pages of a team where the user is a pending admin while the team has no accepted admin, and a notice leads to it otherwise; a Responsibilities section in team settings, where pending admins accept; a pending admin chip in the member list.
