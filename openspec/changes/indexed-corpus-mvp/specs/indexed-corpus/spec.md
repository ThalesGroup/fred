...
### Requirement: A corpus synchronization is accepted, then completed, independently of when its documents become queryable
...
- **WHEN** a synchronization for a corpus instance is accepted
...
#### Scenario: A corpus is queryable incrementally, not only once its synchronization completes
- **GIVEN** a synchronization for a corpus instance is in progress
...
Requesting a synchronization for a corpus instance that already has one accepted and not yet terminal SHALL NOT start a second one.

#### Scenario: Resubmitting after a lost response does not start a second synchronization
- **GIVEN** a synchronization for a corpus instance has already been accepted and is not yet in a terminal state
- **WHEN** the same corpus instance's synchronization is requested again (for example, a client retry after a lost response)
...
- **GIVEN** a corpus instance's most recent synchronization has reached a terminal state
...
- **GIVEN** an item was already durably ingested into a corpus instance under its current source revision
...
### Requirement: A corpus instance's synchronization never writes to its remote source
...
- **WHEN** a synchronization discovers and fetches changes from a corpus instance's remote source
...
- **GIVEN** a corpus type's connector is provisioned with a credential or mount to reach its remote source
...
A request to create, delete, or otherwise mutate a document on a corpus instance's behalf SHALL be checked against three things together — the caller's authorized identity, the corpus bound to the executing synchronization, and the target document's own recorded ownership — and refused if any one of them disagrees. A corpus identifier supplied by the caller SHALL NOT, by itself, establish authorization to mutate that corpus.

#### Scenario: Deleting a document requires its corpus to match both the request and the executing synchronization's own bound scope
- **GIVEN** a document that belongs to one corpus instance
- **AND** a synchronization is executing on behalf of a different corpus instance
- **WHEN** that synchronization issues a delete request naming the first document
- **THEN** the deletion is refused and the document is not removed, regardless of what corpus identifier the request itself carries

#### Scenario: An unauthenticated or unauthorized caller cannot mutate any corpus's documents
- **GIVEN** a request to submit or delete a document, carrying a corpus identifier that does match a real corpus instance
- **WHEN** the caller is not the authorized execution identity for that corpus's current synchronization
...
### Requirement: A corpus type's usage authorization is re-verified on every synchronization, not only at creation
Revoking a team's authorization to use a corpus type SHALL stop future synchronizations of that team's instances without deleting the instances or their content.

#### Scenario: A synchronization does not run once its team's authorization is revoked
- **GIVEN** a corpus instance was created while its team was authorized to use its corpus type
...
- **GIVEN** a corpus instance whose team's authorization to use its corpus type has been revoked
...
A corpus instance's kind SHALL NOT determine which retrieval paths its content can appear in; a document's tag scope alone determines that, identically to a manually uploaded document.

#### Scenario: A corpus instance's documents are retrievable by semantic search
- **GIVEN** a corpus instance whose documents carry its own scope tag
- **WHEN** an agent capability configured for that tag performs a semantic search
- **THEN** the corpus instance's ingested documents are included in the results, through the same retrieval path as a manually uploaded document carrying that tag

#### Scenario: A corpus instance's tabular documents are queryable by SQL
- **GIVEN** a corpus instance whose documents carry its own scope tag and include tabular content
- **WHEN** an agent capability scoped to that tag runs a query over that tabular content
- **THEN** the corpus instance's ingested tabular documents are queried through the same path as a manually uploaded tabular document carrying that tag

#### Scenario: A chat user retrieves an authorized corpus's content without configuring any synchronization detail themselves
- **GIVEN** a team's admin has already authorized the team's corpus type and an agent has already been configured with capability access scoped to that corpus's tag
- **WHEN** a user of that team asks the agent a question answerable from that corpus's content
...
### Requirement: Creating or operating a corpus instance never configures an agent capability
A corpus type or instance SHALL NOT create, modify, or imply any agent capability or tool configuration.

#### Scenario: Creating a corpus instance does not configure any agent
- **WHEN** a team creates a corpus instance
...
- **WHEN** a synchronization emits a progress or terminal event for a corpus instance
- **THEN** that event's content contains no resolved credential and no secret value — only whatever identifies the corpus instance and the item being processed
