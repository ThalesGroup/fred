...
- `frontend-package-archives`: Retire the completed first-release bootstrap, recovery, and retained-artifact verification-continuation gates while requiring a usable preparation-only workflow and preserving generic candidate/registry validation and immutable historical evidence. This is a targeted modification of the release-gate requirement in [add-frontend-package-release-foundations](../add-frontend-package-release-foundations/specs/frontend-package-archives/spec.md), not a parallel release contract. The predecessor is not yet synchronized into the main specification; its remaining task must be resolved before either delta is synchronized.
