...
export function chatMessagesToMessage(
  exchangeMessages: ChatMessage[],
    ...
    if (userMsgs.length > 0) {
      ...
      if (userUiMsg) {
        ...
        if (assistantMsgs.length > 0) {
          ...
          if (assistantUiMsg) {
            messages[assistantUiMsg.id] = assistantUiMsg;
            userUiMsg.childrenIds.push(assistantUiMsg.id);
            userUiMsg.activeChildId = assistantUiMsg.id;
          }
        }
      }
    }
