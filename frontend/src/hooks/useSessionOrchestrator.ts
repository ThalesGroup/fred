...
export function useSessionOrchestrator(params: {
  ...
  useEffect(() => {
    console.log("Orchestrator: useEffect triggered by sessionsFromServer change. New count:", sessionsFromServer?.length);
  ...
  const selectAgenticFlowForCurrentSession = useCallback((flow: AgenticFlow) => {
  ...
  const deleteSession = useCallback((session: SessionSchema) => {
