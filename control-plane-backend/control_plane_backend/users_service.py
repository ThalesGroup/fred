...
from fred_core.users import UserRow, GcuVersionsType
from fred_core import (
  ...
  Resource,
  authorize,
  create_keycloak_admin, BaseUserStore,
)
...
async def find_user_details_by_id(user_id: UUID, user_store: BaseUserStore=Depends(get_user_store)) -> UserRow:
...
async def update_gcu_validation(user_id: UUID, user_store: BaseUserStore=Depends(get_user_store)) -> None:
  cfg = ApplicationContext.get_configuration()
