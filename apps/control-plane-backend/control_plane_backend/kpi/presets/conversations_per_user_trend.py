...
async def query_conversations_per_user_trend(
    ...
    *,
    ...
    # Same metric and dim as the `conversations_per_user` histogram, so the
    # trend and the median tile beside it describe one population; only the
    # question changes, from "over the range" to "over the trailing window".
...
CONVERSATIONS_PER_USER_TREND_PRESET = PresetDef(
    name="conversations_per_user_trend",
    response_model=TimeSeriesResponse,
    handler=query_conversations_per_user_trend,
    summary="Median conversations per active user over a trailing window, bucketed over time",
