...
class PlatformBootstrapStore:
    ...
    async def get_completed_by(self) -> str | None:
        ...
        Read-only counterpart to `is_completed()`. Used by `platform_roles.py`
        to identify the one identity whose `platform_admin` role can never be
        revoked, by itself or anyone else (PLATFORM-ADMIN-DELEGATION-RFC.md).
        """
        async with self._sessions() as s:
            row = await s.get(PlatformBootstrapRow, SINGLETON_ID)
        return row.completed_by if row is not None else None
