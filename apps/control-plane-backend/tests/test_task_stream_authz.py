...
creator, platform admin, or a team reader of the task's team. Regression guard for
the divergence where an admin could list an erasure task (created with the affected
user's uid) but got 403 trying to stream it."""
...
from control_plane_backend.tasks.api import _authorize_task_stream


def _user(uid: str = "u", roles: list[str] | None = None) -> KeycloakUser:
    return KeycloakUser(uid=uid, username=uid, roles=roles or [], email=None, groups=[])
...
class _FakeRebac:
    """Configurable platform/team verdicts; records the team checks it was asked."""
...
@pytest.mark.asyncio
async def test_creator_may_stream_without_consulting_rebac() -> None:
    rebac = _FakeRebac()
    await _authorize_task_stream(
        _user("alice"), _run(created_by="alice", team_id="t1"), cast(Any, rebac)
    )
    assert rebac.team_checks == []  # ownership short-circuits


@pytest.mark.asyncio
async def test_legacy_admin_role_may_stream() -> None:
    rebac = _FakeRebac()
    await _authorize_task_stream(
        _user("sys", roles=["admin"]),
        _run(created_by="alice", team_id="t1"),
        cast(Any, rebac),
    )
...
async def test_platform_admin_may_stream_another_users_task() -> None:
    ...
    await _authorize_task_stream(
        _user("admin-user"), _run(created_by="alice", team_id="t1"), cast(Any, rebac)
    )


@pytest.mark.asyncio
async def test_team_reader_may_stream_team_task() -> None:
    # The core alignment: if a manager can list a team task (CAN_READ_MEMBERS),
    # they can stream it — even though the erasure task's created_by is the user.
    rebac = _FakeRebac(platform=False, team_ok=True)
    await _authorize_task_stream(
        _user("manager"),
        _run(created_by="alice", team_id="northbridge"),
        cast(Any, rebac),
    )
    assert rebac.team_checks == [
        ("manager", TeamPermission.CAN_READ_MEMEBERS, "northbridge")
    ]
