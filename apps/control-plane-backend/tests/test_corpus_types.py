...
"""
`control_plane_backend.corpus_types.catalog` (docs/swift/rfc/INDEXED-CORPUS-RFC.md
...
def _source(corpus_type_id: str, *, enabled: bool = True) -> CorpusTypeConfig:
...
def test_configured_source_serves_enabled_types_only() -> None:
    source = ConfiguredCorpusTypeCatalogSource(
        (_source("local_fs_rag"), _source("parked", enabled=False))
    )

    catalog = source.load()

    assert [item.corpus_type_id for item in catalog.items] == ["local_fs_rag"]


def test_config_inherits_corpus_type_mode_validation() -> None:
    """`CorpusTypeConfig` adds only `enabled` - the mode/connector_kind
    exclusivity invariant is `CorpusType`'s own, not re-implemented here."""
