...
"""Lightweight OpenDocument ``.odt`` -> Markdown extractor for chat attachments.

Fred rationale:
- Mirrors :class:`LiteDocxToMdProcessor`, but for the OpenDocument ``.odt``
  format which the markitdown/python-docx path does not read directly.
- Converts ``.odt`` -> ``.docx`` once with headless LibreOffice and then reuses
...
from knowledge_flow_backend.core.processors.input.common.legacy_office import convert_odt_to_docx
...
class LiteOdtToMdProcessor(BaseLiteMdProcessor):
    """Fast ODT-to-Markdown converter: LibreOffice upgrade + lite DOCX extractor."""

    description = "Fast ODT-to-Markdown converter using LibreOffice (ODT->DOCX) and the lite DOCX extractor."
    ...
    def extract(self, file_path: Path, options: LiteMarkdownOptions | None = None) -> LiteMarkdownResult:
        ...
        # Preserve the original .odt identity and record the source format.
        extras = {**(result.extras or {}), "source_format": "odt"}
