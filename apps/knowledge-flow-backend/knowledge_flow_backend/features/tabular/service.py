...
def forbidden_datasets_message(document_uids: Sequence[str]) -> str:
    """
    Build the 403 message for caller-supplied document uids that were refused.

    Why this exists:
    - ReBAC denies an object that does not exist, so an agent passing a filename
      or a SQL table alias instead of a uid gets a permission error and reads it
      as an identity problem; the message must name the likely cause and the
      tool that returns real uids, without disclosing whether a document exists.
    """
