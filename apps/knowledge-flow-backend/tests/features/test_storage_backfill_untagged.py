...
class _WritableSession:
    """Records `session.add`ed rows so a test can assert what the backfill
    manufactures. `get` reports every lookup as missing, which is the branch
...
@pytest.mark.asyncio
async def test_manufactured_team_row_is_explicitly_public() -> None:
    """#2433: `visibility` must be stated, not left to the ORM default (now
    "private"). A team reaching this branch pre-exists its registry row and
    was behaviorally public under the old regime — inheriting the new default
    would make `_list_teams`' lazy backfill revoke its ReBAC `public` relation
    and silently hide it from the marketplace as a side effect of a storage
    repair."""
    module = _load_backfill_module()
    session = _WritableSession()

    await module.update_database(session, {}, {"team-a": 4096})

    team_rows = [r for r in session.added if type(r).__name__ == "TeamMetadataRow"]
    assert len(team_rows) == 1
    assert team_rows[0].visibility == "public"
    assert team_rows[0].current_resources_storage_size == 4096
