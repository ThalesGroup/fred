...
def _reset_delegation() -> Iterator[None]:
    previous = get_delegation_config().model_copy(deep=True)
    initialize_delegation(DelegationConfig())
    try:
        yield
    finally:
        initialize_delegation(previous)


def test_access_log_removes_delegation_query_parameters(caplog) -> None:
    initialize_delegation(DelegationConfig(enabled=True, allowed_callers=["synthetic-workload"]))
    app = FastAPI()
