# THEME OVERRIDE TEST - this is not a real privacy notice
...
2. It unpacked `gdpr.md` into the overlay directory, outside the web root.
...
`apps/frontend/theme/gdpr.md` and exists only so the archive is complete and
the override is verifiable end to end.

Keep `gdpr.fr.md` beside it. The application asks for `gdpr.<lang>.md` first, and
