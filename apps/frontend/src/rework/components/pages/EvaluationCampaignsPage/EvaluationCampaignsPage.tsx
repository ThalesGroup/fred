...
function CaseDrawer({ campaignId, open, onClose }: { campaignId: string | null; open: boolean; onClose: () => void }) {
  ...
  const handleCaseClick = (c: EvaluationCaseResponse) => {
    onClose();
    navigate(`/settings/evaluations/${campaignId}`, { state: { selectedCaseId: c.case_id } });
  };
      ...
      <CardContent sx={{ pb: "12px !important" }}>
        ...
        {c.execution_error && (
          <Box sx={{ mt: 1, p: 1.5, borderRadius: 1, bgcolor: "#ef444418", border: "1px solid #ef444440" }}>
            ...
            <Typography variant="caption" color="#ef4444" sx={{ display: "block", mt: 0.5 }}>
              {c.execution_error}
            </Typography>
          </Box>
        )}
    ...
    <Container maxWidth="xl" sx={{ py: 3 }}>
      <TopBar title="Évaluations" description="Campagnes de validation d'agents">
        <Stack direction="row" spacing={2} alignItems="center">
          ...
          <Button color="primary" variant="filled" size="medium" onClick={() => navigate("/settings/evaluations/new")}>
            + Nouvelle campagne
          </Button>
        </Stack>
      </TopBar>
      ...
      <Stack direction="row" spacing={2} sx={{ mb: 3 }}>
        ...
        <KpiCard
          ...
          value={globalScore ?? 0}
          color={
            globalScore === null ? "#6b7280" : globalScore >= 80 ? "#22c55e" : globalScore >= 50 ? "#f59e0b" : "#ef4444"
          }
          suffix="%"
        />
          ...
          {!isLoading && !isError && campaigns.length === 0 && (
            <Box sx={{ py: 4 }}>
              <PageEmptyState
                ...
                action={{ label: "Créer une campagne", onClick: () => navigate("/settings/evaluations/new") }}
              />
            </Box>
          )}
                            ...
                            {Object.entries(c.metric_averages).map(([name, avg]) => {
                              ...
                              return (
                                <Stack key={name} direction="row" spacing={1} alignItems="center">
                                  ...
                                  <Typography variant="caption" fontWeight={700} color={color} sx={{ fontSize: 11 }}>
                                    {pct}%
                                  </Typography>
                                </Stack>
                              );
                      ...
                      <TableCell onClick={(e) => e.stopPropagation()}>
                        <Stack direction="row" spacing={1}>
                          <Button
                            ...
                            variant="outlined"
                            size="small"
                            onClick={() => navigate(`/settings/evaluations/${c.campaign_id}`)}
                          >
                            Détail
                          </Button>
                          <Button
      ...
      <CaseDrawer campaignId={drawerCampaignId} open={!!drawerCampaignId} onClose={() => setDrawerCampaignId(null)} />
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            Delete <strong>{deleteTarget?.name}</strong>? This will permanently remove all cases, metrics and events
