        ...
        <>
          ...
          <Box sx={{ overflowY: "auto", flex: 1 }}>
            ...
            {caseData.metrics.length > 0 && (
              <Box>
                <Typography variant="subtitle2" sx={{ mb: 1 }}>
                  Métriques ({caseData.metrics.length})
                </Typography>
                <Stack spacing={1.5}>
                  {caseData.metrics.map((m: EvaluationMetricResultResponse, i: number) => (
                    <Box key={i}>
                      ...
                      {m.score != null && (
                        <ProgressBar
                          theme={
                            m.verdict === "passed" ? "success" : m.verdict === "insufficient" ? "warning" : "error"
                          }
            ...
            {caseData.structural_checks?.length > 0 && (
              <Box>
                <Typography variant="subtitle2" sx={{ mb: 1 }}>
                  Vérifications structurelles ({caseData.structural_checks.length})
                </Typography>
                <Stack spacing={1}>
...
function AnalysisSection({ title, items, color }: { title: string; items: string[]; color: string }) {
  return (
    <Box>
      <Typography
        variant="caption"
        sx={{ color, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", display: "block", mb: 1 }}
      >
        {title}
      </Typography>
      <Stack spacing={0.8}>
    ...
    <Card sx={{ mb: 3, bgcolor: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.10)" }}>
      <CardContent sx={{ p: 3 }}>
        <Stack direction="row" justifyContent="space-between" alignItems="flex-start" sx={{ mb: 2.5 }}>
          ...
          <Button color="secondary" variant="text" size="small" onClick={onClose}>
            ✕
          </Button>
        </Stack>
        ...
        <Typography
          ...
          sx={{ lineHeight: 1.7, mb: 3, borderLeft: "2px solid rgba(255,255,255,0.12)", pl: 2 }}
        >
          {analysis.summary}
        </Typography>
  ...
  if (!campaign) {
    return (
      <Container maxWidth="xl" sx={{ py: 4 }}>
        <Alert severity="error">Campagne introuvable.</Alert>
      </Container>
    );
  }
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 3 }}>
      <TopBar title={campaign.name} description={`Campagne · ${campaign.campaign_id.slice(0, 12)}`}>
        <Stack direction="row" spacing={1} alignItems="center">
          {isLive && (
            <Button color="error" variant="outlined" size="medium" onClick={handleCancel} disabled={isCancelling}>
              {isCancelling ? <CircularProgress size={14} sx={{ mr: 1 }} /> : null}
              Annuler
            </Button>
          )}
          {campaign.operational_state === "completed" && (
            <Button color="secondary" variant="outlined" size="medium" onClick={handleAnalyze} disabled={isAnalyzing}>
          ...
          <Button color="secondary" variant="text" size="medium" onClick={() => navigate("/settings/evaluations")}>
            ← Retour
          </Button>
        </Stack>
      </TopBar>
      ...
      {/* Metadata accordion */}
      <Accordion
        sx={{ mb: 3, bgcolor: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)" }}
        disableGutters
      >
                          ...
                          {c.metrics.slice(0, 2).map((m: EvaluationMetricResultResponse, i: number) => (
                            <Chip
                              ...
                              sx={{
                                ...
                                border: `1px solid ${verdictColor(m.verdict)}40`,
                                bgcolor: `${verdictColor(m.verdict)}18`,
                              }}
                            />
                          ))}
                          {c.metrics.length > 2 && (
                            <Chip label={`+${c.metrics.length - 2}`} size="small" sx={{ fontSize: 10 }} />
                          )}
                        </Stack>
                      ) : (
                        <Typography variant="caption" color="text.secondary">
