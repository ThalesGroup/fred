...
Admin** — the last one can't be removed.
...
- **Platform admin** — governs the **team registry** (which teams exist): list
  all teams, delete one, or "rescue" a team left with no administrator. It also
  seeds the first Admin when a team is created.
- **Platform observer** — accesses **cross-cutting observability**: the
  platform-wide KPIs and analytics (Platform admin inherits this).

> **A platform role never substitutes for a team role.** A Platform admin who
...
- Each role is a **permission recorded on the server**, kept separate from how
  you sign in — signing in only proves who you are, it doesn't by itself grant
  any role.
- Every action is **checked on the server**, not just hidden in the interface:
  hiding a button is never enough to authorize an action.
- Your **personal space** is accessible to you alone: no one, not even a
  Platform admin, can reach it.
