...
- **Document production**
  ...
  - **Generate a web page (HTML/CSS)**: the agent produces a static HTML/CSS
    page, rendered in a preview next to the chat.
...
## How your agent reads documents

Three different packs touch your team's documents, and it's worth knowing
what each one actually gives you. You never pick between them yourself in a
conversation — the agent chooses automatically based on what you ask — but
knowing the difference helps you turn on the right pack and trust the answer.
...
- **Summarize** (part of _Access to team resources_ and _Attachments in a
  conversation_): a short, readable overview. It's deliberately not
